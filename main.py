...
def get_upcoming_events(date):
    response = requests.get(f"https://zsr.octane.gg/events?after={date}")
...
def get_upcoming_matches(date):
    response = requests.get(f"https://zsr.octane.gg/matches?after={date}")
    r_json = response.json()
    print(f"Response status : {response.status_code}\n")
...
def get_match_games(match_id):
    response = requests.get(f"https://zsr.octane.gg/matches{match_id}/games")
    r_json = response.json()
    print(f"Response status : {response.status_code}\n")
